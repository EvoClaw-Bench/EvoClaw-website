...
:root{
   ...
   * --moonshot-tag-text mirrors --zai-tag-text: contrast colour for
   * text painted INSIDE the NEW tag (which uses --moonshot-accent as
   * background). */
  --moonshot-accent:#C8CBD3;
  --moonshot-tag-text:#1a1d2e;
}
[data-theme="light"]{
...
.container{max-width:1580px;margin:0 auto;padding:2.5rem 1.5rem}
...
/* Per-banner accent set via inline --accent so each NEW row can use the
...
.news-banner--moonshot{--accent:var(--moonshot-accent);--tag-text:var(--moonshot-tag-text)}
...
function renderChart() {
  ...
  const traces = orgs.map(org => {
    ...
    return {
      ...
      // Hover highlight — when plotly_hover sets selectedpoints to a single
      // index, that one marker grows into a soft purple halo. Other org
      // points stay invisible (transparent).
      selected: { marker: { size: 56, color: 'rgba(167,139,250,0.18)' } },
      unselected: { marker: { color: 'rgba(0,0,0,0)' } },
  ...
  // Hover highlight — when the user hovers a data point, mark it as
  // selected so the org trace's `selected.marker` styling kicks in
  // (soft purple halo). On unhover, clear the selection so the halo
  // disappears.
  let hoverCurve = -1;
  chartEl.on('plotly_hover', function(ev) {
    if (!ev || !ev.points || !ev.points.length) return;
    const pt = ev.points[0];
    // Frontier line + centre-dot traces have no customdata — skip them.
    if (!pt.data || !pt.data.customdata) return;
    hoverCurve = pt.curveNumber;
    Plotly.restyle(chartEl, { selectedpoints: [[pt.pointIndex]] }, [hoverCurve]);
  });
  chartEl.on('plotly_unhover', function() {
    if (hoverCurve < 0) return;
    Plotly.restyle(chartEl, { selectedpoints: [null] }, [hoverCurve]);
    hoverCurve = -1;
  });
...
function isWorst(val, key, fdata) {
  if (HIGHER.has(key)) return Math.abs(val - Math.min(...fdata.map(d => d[key]))) < 0.005;
  if (LOWER.has(key))  return Math.abs(val - Math.max(...fdata.map(d => d[key]))) < 0.005;
  return false;
}
function valCls(val, key, fdata) {
  if (isBest(val, key, fdata)) return ' best-val';
  if (isWorst(val, key, fdata)) return ' worst-val';
  return '';
}
...
function renderTable() {
  ...
  function numCell(v, k, dec) {
    return '<td class="num' + valCls(v, k, fdata) + '">' + fmtNum(v, dec) + '</td>';
  }
  ...
  tbody.innerHTML = sorted.map(d => {
    ...
    return '<tr class="' + topCls + '">' +
      ...
      '<td class="score-cell"><div class="score-bar" style="width:' + barW + '%;background:' + d.agent_fg + '"></div><span class="score-val' + valCls(d.score,'score',fdata) + '">' + fmtNum(d.score, 2) + '</span></td>' +
      numCell(d.precision, 'precision', 2) +
      numCell(d.recall, 'recall', 2) +
      numCell(d.resolve, 'resolve', 2) +
      numCell(d.cost, 'cost', 2) +
      '<td class="num' + valCls(d.out_tok_k,'out_tok_k',fdata) + '">' + d.out_tok_k.toLocaleString() + '</td>' +
      numCell(d.time_h, 'time_h', 2) +
      '<td class="num' + valCls(d.turns,'turns',fdata) + '">' + d.turns.toLocaleString() + '</td>' +
    '</tr>';
  }).join('');
}
