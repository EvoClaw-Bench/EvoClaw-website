...
function renderChart() {
  ...
  const TIER = {
    ...
    near: { dotSize: 5, dotOpacity: 1.0,  pillAlphaMult: 1.0, iconOpacity: 1.0,  labelAlpha: 0.78 },
  ...
  const annotations = fdata.map(d => {
    ...
    // Bold the model name on both frontier (purple, full size) and near-
    // frontier tiers so they pop against the muted off-tier labels — the
    // colour + size still distinguish on from near visually.
    const boldName = onFrontier || tierKey === 'near';
    const label = boldName
